...
class JogosEliteModule:
    ...
    async def execute(self):
        """Executa o monitoramento de jogos de elite"""
        ...
        try:
            ...
            for days_ahead in range(2):
                ...
                matches = self.api_client.get_fixtures_by_date(date_str)
            ...
            for match in all_matches:
                try:
                    ...
                    season = self.api_client.get_current_season(league_id)
                    
                    qualifying_teams = []
                    
                    # Verificar time da casa
                    if self.normalize_name(home_team) in self.elite_teams_normalized:
                        team_id = self.api_client.get_team_id(home_team)
                        if team_id:
                            avg = self.api_client.get_team_goals_average(team_id, league_id, season)
                    ...
                    if self.normalize_name(away_team) in self.elite_teams_normalized:
                        team_id = self.api_client.get_team_id(away_team)
                        if team_id:
                            avg = self.api_client.get_team_goals_average(team_id, league_id, season)
                            if avg is not None and avg >= Config.ELITE_GOALS_THRESHOLD:
